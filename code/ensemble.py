...
def __save_ensemble(prediction: OrderedDict, output_dir: str, postfix: Optional[str] = None) -> NoReturn:
    try:
...
def __hard_voting(nbest_json: List[Dict[str, List[Dict]]], id_list: List[str]) -> OrderedDict:
...
def __soft_voting(nbest_json: List[Dict[str, List[Dict]]], id_list: List[str]) -> OrderedDict:
    prediction = OrderedDict()
