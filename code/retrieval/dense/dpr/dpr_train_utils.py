...
import os
from .dpr_model import DPRetrieval, Encoder
import pickle
import torch
import tqdm
import numpy as np
...
class DPRTrainer(DPRetrieval):
    """Class used for Dense Passage Retriever train
    """
    ...
    def load_passage_embedding(self):
        """ Load the saved passage_embedding
        """
        if path.isfile(path.join(self.args.p_encoder_path, "passage_embedding.bin")):
            ...
            if "p_encoder.pt" in os.listdir(self.args.p_encoder_path):
                self.p_encoder.load_state_dict(
                    torch.load(self.args.p_encoder_path + "/p_encoder.pt")
                )
            else:
                raise f"p_encoder.pt 파일이 {self.args.p_encoder_path} 폴더 안에 있지 않습니다."
    ...
    def set_embedding(self, queries, q_encoder=None, p_encoder=None):
        """Embedding question and passage based on given q_encoder and p_encoder
    ...
    def train(self, args, **kwargs):
        ...
        Args:
            args: retriever arguments
            **kwarg : If you use wandb, enter `project`, `entity`, `name`
        """
    ...
    def eval(self, p_encoder, q_encoder, global_step):
        """Performance evaluation of retriever

        Args:
            p_encoder ([type]): trained passage encoder
            q_encoder ([type]): trained question encoder
            global_step (int): training global step

        Returns:
            acc (float): value of precision
        """
